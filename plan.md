...
- [x] 第 16/17 次完整链路因外部输入缺口、接收抓包超额及 VLC 迟到未通过；第 22 次改用目标机本地高规格连续源后完成最终验收，不把此前持续运行达标冒充完整通过。
- [x] 第 18 次在不改核心的条件下完成输入证据对照：目标机入口约 2.48 秒无 RTP 时，源服务视频帧计数仍增加 51 帧；缺包位于源服务 RTP 发出端或到目标机的传输路径，最终区分需要源机/中间设备同时段抓包。
...
- [x] 第 22 次使用目标机已有高规格视频制作有限 H.264 源，原参数 RKMPP H.264 2560x1440@30 RTP → HEVC 1920x1080@25 CBR 6 Mbps MPEG-TS/RTP 连续运行 248 秒；源流期间核心不停，发出端服务曲线超额 1356 B，收发 RTP payload 哈希一致，接收端 6204 帧全解码，VLC URL 软件解码无 deadlock、晚帧或解码错误。
- [x] 将第 22 次完整验收形成独立成功测试提交并推送；随后冻结代码并完成两名未参与实现的独立审查与 PR 复审。
