...
{
    const auto options = parse({ "build-week", "demo", "--input", "input.mp4" });
...
void testInspectOverrides()
{
    const auto options = parse({
        "build-week", "inspect", "--input", "clip.mov", "--output", "planned.mp4",
        "--video-codec", "hevc", "--audio-codec", "opus", "--width", "1280",
        "--height", "720", "--bitrate", "2500", "--disable-hw", "--no-audio",
        "--quiet-graph"
    });
    ...
    assert(options.width == 1280 && options.height == 720);
    assert(options.videoBitrateKbps == 2500);
    assert(!options.includeAudio && options.disableHardware && !options.diagnosticLogEnabled);
}

void testLiveDefaults()
{
    const auto options = parse({ "build-week", "live", "--input", "rtsp://camera/live" });
...
void testLiveDurationOverride()
{
    const auto options = parse({
        ...
        "--output", "udp://192.168.1.20:7354?pkt_size=1316", "--duration", "30"
    });
    assert(options.durationSeconds == 30);
    assert(options.output == "udp://192.168.1.20:7354?pkt_size=1316");
}

void testGraphPresentation()
{
    MediaGraph graph;
    const MediaNodeId input = graph.addNode(MediaNodeKind::FileInput, "input");
    const MediaNodeId encoder = graph.addNode(MediaNodeKind::VideoEncode, "video-encode");
    assert(graph.setNodeOption(encoder, "pipeline.chain", "CUDA zero-copy"));
    assert(graph.setNodeOption(encoder, "decoder.pipeline.ffmpeg", "h264_cuvid"));
    assert(graph.setNodeOption(encoder, "filter.pipeline.filter", "scale_cuda"));
    assert(graph.setNodeOption(encoder, "encoder.pipeline.ffmpeg", "h264_nvenc"));
    assert(graph.setNodeOption(encoder, "pipeline.score", "980"));
    assert(graph.setNodeOption(encoder, "pipeline.zero_copy", "1"));
    assert(graph.setNodeOption(encoder, "pipeline.all_hardware", "1"));
    (void)input;

    const std::string graphText = formatBuildWeekGraph(graph);
    assert(graphText.find("nodes          2") != std::string::npos);
    assert(graphText.find("input") != std::string::npos);
    assert(graphText.find("video-encode") != std::string::npos);

    const std::string planText = formatBuildWeekPlan(graph);
    assert(planText.find("CUDA zero-copy") != std::string::npos);
    assert(planText.find("h264_cuvid") != std::string::npos);
    assert(planText.find("scale_cuda") != std::string::npos);
    assert(planText.find("h264_nvenc") != std::string::npos);
    assert(planText.find("980") != std::string::npos);
}
