...
bool FFmpegVideoPipeline::initialize(const Config& config, std::string* error)
{
    ...
    if (!config.encoderCtx) {
        if (error) {
            *error = "FFmpegVideoPipeline initialize failed: encoderCtx is null";
        }
        return false;
    }
    ...
    if (!config.outputFmtCtx) {
        if (error) {
            *error = "FFmpegVideoPipeline initialize failed: outputFmtCtx is null";
        }
        return false;
    }
    ...
    if (!config.outputVideoStream) {
        if (error) {
            *error = "FFmpegVideoPipeline initialize failed: outputVideoStream is null";
        }
        return false;
    }
...
bool FFmpegVideoPipeline::sendFrame(AVFrame* frame, std::string* error)
{
    if (!m_encoderCtx) {
        if (error) {
            *error = "FFmpegVideoPipeline sendFrame failed: encoderCtx is null";
        }
        return false;
    }
...
int FFmpegVideoPipeline::receiveAndWritePackets(
    std::string* error,
    const PacketWrittenCallback& onPacketWritten)
{
    if (!m_encoderCtx || !m_outputFmtCtx || !m_outputVideoStream) {
        if (error) {
            *error = "FFmpegVideoPipeline receiveAndWritePackets failed: pipeline is not initialized";
        }
        return -1;
    }
...
int64_t FFmpegVideoPipeline::packetCount() const
{
    return m_packetCount;
}

int64_t FFmpegVideoPipeline::lastWrittenOutTimeMs() const
{
