...
Status FFmpegFileOutputSession::initialize(Config config)
{
    ...
    muxerConfig.outputFmtCtx = m_outputFmtCtx.get();

    Status status = m_muxerOutputNode.initialize(muxerConfig);
    if (!status) {
        reset();
        return status;
    }

    status = m_outputGraphController.attachExternalNode(&m_muxerOutputNode);
    if (!status) {
        reset();
        return status;
    }
