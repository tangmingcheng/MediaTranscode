    ...
    const char* stream)
{
    if (packetBytes == 0 || packetBytes > maximumPacketBytes ||
        packets >= packetCapacity || packetBytes > byteCapacity - bytes) {
        return ::media::Status::failure(::media::ErrorInfo::invalidArgument(
            std::string("prepared generic input exceeds planned ") + stream +
            " replay bounds"));
    }
    ++packets;
    bytes += packetBytes;
...
struct MediaPreparedGenericInput::CaptureState final {
    ...
    CaptureState(
        ...
        std::size_t preparedVideoPackets,
        std::size_t preparedAudioPackets,
        std::uint64_t preparedVideoBytes,
        std::uint64_t preparedAudioBytes)
        : context(std::move(ownedContext)),
          replay(std::move(preparedReplay)),
          plan(preparedPlan),
          nextOrdinal(nextPreparedOrdinal),
          videoPackets(preparedVideoPackets),
          audioPackets(preparedAudioPackets),
          videoBytes(preparedVideoBytes),
          audioBytes(preparedAudioBytes)
    {
    ...
    MediaAvSyncStartupPolicy startup)
{
    ...
    auto capture = std::make_unique<CaptureState>(
        ...
        videoPackets, audioPackets, videoBytes, audioBytes);
    capture->start();
