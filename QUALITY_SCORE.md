...
范围：`a5597326` 至本轮的 23 个生产文件；历史全仓评分保留，本表不代表全库重评。第 22 次使用目标机本地高规格有限源完成 248 秒原参数 RKMPP 实流验收；代码冻结后的最终交叉审查以本轮最新提交为准。
...
| 真实验收证据 | 19/20 | RKMPP 本地高规格源持续 248 秒；源流期间核心不停，发送服务曲线超额 1356 B，收发 RTP payload 哈希一致，RTP/TS 错误为 0，接收端 6204 帧全解码，VLC URL 软件解码无晚帧或错误。缺少多小时 soak。 |
| **专项合计** | **90/100** | **本地高规格源完整链路 PASS；长期与平台覆盖风险保留。** |

残余风险：Linux TX timestamp telemetry 当前未跟踪；Windows 接收抓包仍观察到 `17387.5 B` 到达聚合，虽然发送端为 `1356 B` 且收发数据完全一致；裸 RTP 源自然结束后仍以 source-clock expiry 退出码 1 收敛；VLC D3D11VA 路径仍会卡住，本次使用软件解码通过；按用户要求未复测最终 shared batch-prefix 的 Windows 转码；248 秒验收不能替代多小时硬件 soak。另有未物化尾部等待不计入 wire residence、LOW_DELAY 阻塞取包在硬件失去响应时可能阻塞 worker 的设计风险。详见 `docs/rk-a559-external-rtp-validation.md`。
