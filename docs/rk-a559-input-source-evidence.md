...
第 18 次测试没有修改核心代码。第 16/17 次已归档二进制 SHA256
`96c5c71d3183cfe11846dba36ee48f4116983c775e7caa1b3eb5078188989b57`；
第 18 次证据目录没有再次记录二进制哈希，因此不把该哈希作为第 18 次的独立证明。
...
`1250757`、driver PID `1250754`；这三个 PID 均由归档的 `pids.txt` 支持。
...
| 无包时长 | `2478.929 ms` |
| 同窗口采样脚本记录的 `frames` 字段 | `2671632 -> 2671658 -> 2671683` |
| 源服务采样跨度 | `2010.226 ms`，增加 `51` 帧 |
| 三次 API 响应耗时 | `3.931 ms`、`3.958 ms`、`4.039 ms` |

归档 JSON 没有保留原始 API 响应、采样 URL 和 stream 标识，不能独立证明
`frames` 字段属于指定流的视频轨；该计数只作为辅助关联。目标机入口连续约
2.48 秒没有收到指定 SSRC/PT 的 RTP 流，由入口抓包本身直接证明。

输入序号分析另记录 `21` 个序号缺口，全部为从 `N` 跳到 `N+2`；归档 JSON
没有单独记录迟到或乱序计数，因此不据此排除迟到或乱序。例如
`16:56:08.488555 +08:00`，同一
