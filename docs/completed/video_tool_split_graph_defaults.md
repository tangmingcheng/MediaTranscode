...
```powershell
D:\mabs\local64\bin-video\ffmpeg.exe -hide_banner -loglevel error -re -stream_loop -1 -i out\build\x64-debug\test.mp4 -map 0:v:0 -an -c:v copy -f rtp -payload_type 96 "rtp://127.0.0.1:5004?pkt_size=1200"
...
Result: exit code `0`; selected `filter=not_required`, final report had `encodedPacketsPushed=34`, `encodedPacketsPopped=34`, `workerErrors=0`.
