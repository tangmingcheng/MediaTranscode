...
目标机正在全量构建 build9，尚未完成此修改的真实 CLI 回归。两位独立审查者均因服务额度限制中断；没有最终双 PASS。共享输入节点对 Windows 有潜在影响，按用户要求不运行 Windows 媒体验收。

用户随后明确要求立即出库、出完继续处理剩余问题，因此当前库交付不再等待完整验收与审查，但必须标为待验收版本。使用目标机原有 `/home/tang/package_media_transcode_beta.sh <release-build-dir> <package-dir>`，不另建打包流程；发送队列突发、四路回归及双独立复审继续保留为未完成项。
