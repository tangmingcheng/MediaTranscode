...
后续 run43 已完成恢复后 190.04 秒连续硬解、收发零丢包及发送一包上限，完整门禁 PASS；已交付独立新版库。冻结 2d597ba6 现由两位未参与实现者重新明确 Standards PASS / Spec 源码 PASS，专项评分各 88/100。run44 反向 CBR 同样完整通过；两项 VBR、旧内核突发根因及特定 WouldBlock 分支实测覆盖仍未闭合。
