...
输入源发往目标机自身地址，经过 lo；正常编码出流后等待 20 秒，再注入 30 秒丢包。实际注入与恢复命令（eth0 未注入丢包）：
...
- **仍有发送突发**：两端 RTP 均为 136,102，序号和 TS 连续性错误均为 0，RTP 内容流 SHA-256 均为 `79540b233ead44b9fa74d57c526b569126ff8e423c13166c07cac3b6053554961`。发送服务曲线超额 24,317.75 B，超过一包 1,356 B 的既有门限；序号 43700–43718 的 19 包集中在 81 µs 内，发送抓包时间没有倒退。接收服务曲线超额 27,196 B 的最坏事件发生在另一处：序号 28456–28496 的 41 包集中在 4.544 ms 内，对应发送端主要保持约 310–330 µs 包间隔，说明网络或接收端还存在额外聚集。发送器每次只提交一包，按实际 submitCompletedAt 推进 pacing；尚无证据将发送突发归因于用户态追赶，下一轮使用内核现成网络 tracepoint 对照入队与驱动发送时间。
