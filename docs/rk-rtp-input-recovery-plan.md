...
- [x] run47 捕获完整1402 B RTP、槽长1400 B及同毫秒截断退出，确认为核心将样本峰值误作接收上限；源启动成功，核心退出后才停止源。
- [x] 73ad5fbd 复用 prepared transport 的既有协议容量事实传给 ingress planner，保留硬预算与截断拒绝；没有新参数。
- [x] 目标机8核构建与run48完成：输入240.06秒、默认VLC连续硬解228.36秒，140575 RTP全量一致，合法1402 B输入无截断；容量修复双独立源码PASS。完整无突发门禁仍FAIL，24包/182微秒内核聚集未解决。
- [x] 按用户明确指令先交付73ad5fbd容量修复库，包内明确完整门禁FAIL；见2026-09-08交付报告。
...
- [x] run55同容量HEVC 2K30 → H.264 1080p25 CBR6M损伤回归PASS；本地源补充证据，不冒充指定真实流。
- [x] run56真实H.264 1080p25 → HEVC 1080p25 VBR 5/12/13 Mbps损伤回归PASS，连续硬解189.64秒、186422 RTP全量一致、核心发送单包余量。
- [x] run57本地HEVC 1440p30 → H.264 1080p25 VBR 5/12/13 Mbps损伤回归PASS，连续硬解198.01秒、266097 RTP全量一致、核心发送单包余量。输入抓包自身遗漏单列，不冒充输入捕获完整。

用户明确H.264输入使用指定真实流，真实源没有HEVC，因此HEVC输入允许使用已有本地高规格视频。接续未完成项目，不重测已完成CBR。run56和run57已完成两个VBR方向损伤回归；用户要求测试及完整独立交叉审查通过后合并master。内核入队后聚集按用户更新边界保留为部署风险，当前不继续扩大内核优化范围。