...
- [x] 冻结 2d597ba6，两名未参与实现的智能体按行业方案重新独立复审，均明确 Standards PASS / Spec 源码 PASS；已保留两份 88/100 专项评分。
- [ ] 完成剩余矩阵后更新 PR，由独立智能体审核最终 PR。
...
- [x] run43：同一 build9、原参数 H.264 2K30 → HEVC 1080p25 CBR 6 Mbps，恢复后 VLC 连续硬解 190.04 秒、收发 137877 包完全一致、发送抓包及核心内核事件节奏均一包上限，本项 PASS。立即独立提交并产库；双独立源码复审现已通过，旧内核突发根因仍待闭环。见 [run43 报告](completed/2026-09-07-rk-a559-input-recovery-run43.md)。

- [x] run44 HEVC 2K30 → H.264 1080p25 CBR 6 Mbps 输入恢复完整 PASS；连续硬解 190.89 秒、收发 137803 RTP 完全一致、发送一包上限，已独立提交并推送 5197f73a。

## 当前矩阵（冻结生产代码 2d597ba6）

| 输入 RTP → 输出 MPEG-TS/RTP | 输出规格 | 结果 / 独立提交 |
| --- | --- | --- |
| H.264 2K30 → HEVC | 1080p25 CBR 6 Mbps | run43 PASS，ffed7187 |
| HEVC 2K30 → H.264 | 1080p25 CBR 6 Mbps | run44 PASS，5197f73a |
| H.264 2K30 → HEVC | 1080p25 VBR 5/12/13 Mbps | run45 源视频链路 PASS，3b8f5062；EOF 后 283 个无视频负载维护 RTP 未被接收抓包覆盖 |
| HEVC 2K30 → H.264 | 1080p25 VBR 5/12/13 Mbps | run46 FAIL：恢复后连续硬解 191.14 秒、收发 266162 RTP 完全一致，但内核入队后 16 包聚集，发送超额 20896 B |

四项均正常启动后注入 30 秒 20% 输入丢包、源自然结束。不能将当前矩阵标记为全部通过。run46 失败位置已由 pcap 与内核事件闭合，具体停队列/唤醒原因仍待证据；不改输入源或降低发送门禁。

双独立源码审查与独立 PR 的源码/文档审查已明确 PASS，恢复专项评分各 88/100。最终验收因 run46 仍未通过，PR 保持 Draft。库与 C 示例只保留目标机；现有包对应已通过的 run43 二进制，本轮没有生产代码变更。
