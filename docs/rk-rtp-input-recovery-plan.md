...
用户明确H.264输入使用指定真实流，真实源没有HEVC，因此HEVC输入允许使用已有本地高规格视频。接续未完成项目，不重测已完成CBR。run56继续真实H.264 → HEVC VBR 5/12/13 Mbps损伤回归，随后本地HEVC → H.264 VBR。内核入队后聚集按用户更新边界保留为部署风险，当前不继续扩大内核优化范围。