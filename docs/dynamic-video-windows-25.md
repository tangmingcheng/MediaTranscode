...
所有解析序列严格前进。PCR到包偏移跨度分别56.3067/54.1499/48.0149/101.3045ms，恢复路包含源结束后的PCR维护尾部。这些网络证据不足以证明VLC内部调度正确，也不能直接解释369ms积压。后续先按用户要求使用Windows26/27日志及抓包诊断；第27轮仍丢帧后，用户明确授权一次管理员WPR CPU/GPU采集。禁止调大cache、关闭硬解或随机重跑代替定位。
