...
适用版本为目标机已核对的 FFmpeg-rockchip d90e3a1 与实际 MPP c08762ebf。adapter 沿用已有编解码器名称选择机制，权威标识不是运行时二进制版本探测；替换依赖版本需要重新验证源码和实际库。Windows native/CUVID 驻留路径及 NVENC 策略保持原逻辑，共享 planner 和 DAG 仍须 Windows 回归。
...
因此 adapter 记录公共 handoff 一个、私有 last_pkt 一个；公共 BSF 别名不重复计费。只适用于无 FFmpeg frame workers 的 h264_rkmpp/hevc_rkmpp。既有 planner 再组合原子 AU 准备和节点 pending 容量；MPP 内部副本不冒充 engine AVBuffer 驻留，也不声称其驱动字节受 engine 账本硬控。
