...
$BuildDirectory = Join-Path $RepoRoot "out\build\x64-$ConfigurationDirectory"
$InstallPrefix = "D:/Code/MyCode/MediaTranscode/out/install/x64-$ConfigurationDirectory"
...
$buildArguments = @('--build', '.', '--config', $Configuration, '--clean-first', '--target', 'all', '--parallel')
...
Write-Host "VS2026 $Configuration clean-first all-target rebuild succeeded; configure/build exit codes were 0."
exit 0
