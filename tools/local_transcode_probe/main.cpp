...
void printUsage(const char* executable)
{
    std::cout
        << "Usage:\n"
        << "  " << executable << " [options]\n\n"
        << "Local video transcode test tool. This tool is intended for validating the local-file transcode path.\n\n"
        << "Required options:\n"
...
LocalProbeOptions parseOptions(int argc, char* argv[])
{
    LocalProbeOptions options;
...
{
    auto logger = spdlog::stdout_color_mt("local_transcode_probe");
    spdlog::set_default_logger(logger);
    spdlog::set_level(spdlog::level::info);

    try {
        const LocalProbeOptions options = parseOptions(argc, argv);
        if (options.showHelp) {
            printUsage(argv[0]);
            return 0;
        }
